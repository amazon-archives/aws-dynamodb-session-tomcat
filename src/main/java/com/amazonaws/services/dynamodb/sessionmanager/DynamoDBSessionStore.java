...
public class DynamoDBSessionStore extends StoreBase {
	...
	private Set<String> keys = Collections
		...
		new Thread("dynamodb-session-manager-clear") {
			...
			public void run() {
				for (String sessionId : keysCopy) {
					DynamoUtils.deleteSession(dynamo, sessionTableName,
							sessionId);
				}
			}
		}.start();
	...
	public int getSize() throws IOException {
		...
		TableDescription table = dynamo.describeTable(
				new DescribeTableRequest().withTableName(sessionTableName))
				.getTable();
	...
	public Session load(String id) throws ClassNotFoundException, IOException {
		Map<String, AttributeValue> item = DynamoUtils.loadItemBySessionId(
				dynamo, sessionTableName, id);
		if (item == null
				|| !item.containsKey(SessionTableAttributes.SESSION_ID_KEY)
				|| !item.containsKey(SessionTableAttributes.SESSION_DATA_ATTRIBUTE)) {
			DynamoDBSessionManager
					.warn("Unable to load session attributes for session " + id);
			return null;
		}

		Session session = getManager().createSession(id);
		session.setCreationTime(Long.parseLong(item.get(
				SessionTableAttributes.CREATED_AT_ATTRIBUTE).getN()));

		ByteBuffer byteBuffer = item.get(
				SessionTableAttributes.SESSION_DATA_ATTRIBUTE).getB();
		ByteArrayInputStream inputStream = new ByteArrayInputStream(
				byteBuffer.array());

		Object readObject;

		try (CustomObjectInputStream objectInputStream = new CustomObjectInputStream(
				inputStream, getManager().getContext().getLoader()
						.getClassLoader())) {
		...
		if (readObject instanceof Map<?, ?>) {
			...
			for (String s : sessionAttributeMap.keySet()) {
				((StandardSession) session).setAttribute(s,
						sessionAttributeMap.get(s));
			}
		} else {
			throw new RuntimeException(
					"Error: Unable to unmarshall session attributes from DynamoDB store");
		}
